...
const SubscribeModal = () => {
  ...
  const DISMISSED_KEY = 'subscribe_prompt_dismissed_at';

  useEffect(() => {
    const isSubscribed = localStorage.getItem(SUBSCRIBED_KEY);
    const dismissedAt = Number(localStorage.getItem(DISMISSED_KEY) || 0);
    const coolDownPassed = Date.now() - dismissedAt > 24 * 60 * 60 * 1000;
    if (!isSubscribed && coolDownPassed) {
  ...
  const handleSubscribe = async (e) => {
    ...
    try {
      ...
      if (/already subscribed/i.test(backendMessage)) {
        localStorage.setItem(SUBSCRIBED_KEY, 'true');
