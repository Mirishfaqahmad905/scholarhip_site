...
const UnderGraduateScholarship = () => {
  ...
  // Filter for category "undergraduate" (case-insensitive, supports string or array)
  const undergraduateScholarships = (data || []).filter(sch => {
    if (!sch.category) return false;
    if (Array.isArray(sch.category)) {
      return sch.category.some(c => typeof c === 'string' && c.toLowerCase().includes('undergraduate'));
    }
    return typeof sch.category === 'string' && sch.category.toLowerCase().includes('undergraduate');
  });
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gradient-to-br from-blue-50 to-blue-100">
      ...
      {/* Center: Undergraduate Scholarships */}
      <main className="flex-1 p-4">
        <h1 className="text-3xl font-extrabold mb-8 text-center text-blue-800">Undergraduate Scholarships</h1>
        {loading ? (
          <div className="text-center text-gray-500">Loading...</div>
        ) : undergraduateScholarships.length > 0 ? (
          <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {undergraduateScholarships.map((sch, index) => (
              <div
                key={sch._id || sch.id || index}
                className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center border-2 border-blue-100 hover:shadow-2xl transition"
              >
                <img
                  src={sch.image}
                  alt={sch.name}
          ...
          <div className="text-center text-gray-500">No Undergraduate scholarships found.</div>
        )}
      </main>
...
export default UnderGraduateScholarship;