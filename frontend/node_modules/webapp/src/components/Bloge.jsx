...
const Bloge = () => {
  ...
  const resolveMediaUrl = (value) => {
    return Api_url.buildMediaUrl(value);
  };

  const getFirstAvailableImage = (blog) => {
    const contentImage = blog.content?.find((c) => c.type === 'image' && c.value);
    if (contentImage?.value) return resolveMediaUrl(contentImage.value);
    if (Array.isArray(blog.image_urls) && blog.image_urls.length > 0 && blog.image_urls[0]) return resolveMediaUrl(blog.image_urls[0]);
              ...
              {selectedBlog.content.map((block, idx) => {
                ...
                if (block.type === 'image' && block.value) {
                  return (
                    <img
                      key={idx}
                      src={resolveMediaUrl(block.value)}
                      alt="blog content"
