...
const Add_bloge = () => {
  ...
  const addField = (type) => {
    const newField = {
      id: Date.now(),
      type,
      value: type === 'image' || type === 'image_url' ? null : ''
    };
  ...
  const handleSubmit = async () => {
    try {
      ...
      fields.forEach(field => {
        if ((field.type === 'image' || field.type === 'image_url') && field.value) {
          formData.append('images', field.value);
          if (field.type === 'image_url' && !mainImage) {
            mainImage = field.value.name; // we'll replace this later in backend
          }
      ...
      formData.append('mainImageName', mainImage || ''); // pass main image filename
      ...
      <div className="p-6 max-w-4xl mx-auto">
        ...
        {step === 1 && (
          <div className="space-y-4">
            <input type="text" name="title" value={blogMeta.title} onChange={handleMetaChange} placeholder="Blog Title" className="w-full border p-2 rounded" required />
            <input type="text" name="author" value={blogMeta.author} onChange={handleMetaChange} placeholder="Author Name" className="w-full border p-2 rounded" />
            <select name="category" value={blogMeta.category} onChange={handleMetaChange} className="w-full border p-2 rounded">
        ...
        {step === 2 && (
          <>
            <div className="flex gap-2 mb-4 flex-wrap">
              ...
              <button onClick={() => addField('image_url')} className="bg-indigo-500 text-white px-3 py-1 rounded">+ Main Image</button>
            ...
            <div className="space-y-4">
              {fields.map((field) => (
                <div key={field.id} className="relative border p-3 rounded">
                  ...
                  {(field.type === 'image' || field.type === 'image_url') && (
                    <input type="file" accept="image/*" onChange={(e) => updateFieldValue(field.id, e.target.files[0])} className="w-full" />
                  )}
                  {field.type === 'image_url' && <p className="text-sm mt-2 text-indigo-600">📌 This will be used as the main image for the blog (image_url).</p>}
                </div>
              ))}
            </div>
          ...
          <div className="border p-6 rounded shadow bg-gray-50">
            ...
            <div className="space-y-4">
              {fields.map((field) => {
                ...
                if (field.type === 'image' || field.type === 'image_url') return field.value ? <img key={field.id} src={URL.createObjectURL(field.value)} alt="Uploaded" className="w-full max-h-96 object-cover rounded" /> : null;
                return null;
              })}
            </div>
            <div className="flex justify-between mt-6">
