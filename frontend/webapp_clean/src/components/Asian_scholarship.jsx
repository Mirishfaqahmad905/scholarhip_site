...
const Asian_scholarship = () => {
  ...
  // Filter for "Asia" region, case-insensitive, handles string or array
  const scholarshipData = (data || []).filter((item) => {
    if (typeof item.region === 'string') {
      return item.region.trim().toLowerCase() === 'asia';
    }
    if (Array.isArray(item.region)) {
      return item.region.some(
        (r) => typeof r === 'string' && r.trim().toLowerCase() === 'asia'
      );
    }
    return false;
  });
...
export default Asian_scholarship;