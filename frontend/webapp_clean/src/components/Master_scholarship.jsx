...
const Master_scholarship = () => {
  ...
  // Filter for category "master" (case-insensitive, supports string or array)
  const masterScholarships = (data || []).filter(sch => {
    if (!sch.category) return false;
    if (Array.isArray(sch.category)) {
      return sch.category.some(c => typeof c === 'string' && c.toLowerCase().includes('master'));
    }
    return typeof sch.category === 'string' && sch.category.toLowerCase().includes('master');
  });
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gradient-to-br from-blue-50 to-blue-100">
      ...
      {/* Center: Master Scholarships */}
      <main className="flex-1 p-4">
        <h1 className="text-3xl font-extrabold mb-8 text-center text-blue-800">Master Scholarships</h1>
        {loading ? (
          <div className="text-center text-gray-500">Loading...</div>
        ) : masterScholarships.length > 0 ? (
          <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {masterScholarships.map((sch, index) => (
          ...
          <div className="text-center text-gray-500">No Master scholarships found.</div>
        )}
      </main>
...
export default Master_scholarship;